...
func NewCandidate(ctx context.Context, kubeClient client.Client, recorder events.Recorder, clk clock.Clock, node *state.StateNode, pdbs pdb.Limits,
	...
	//data, _ = json.Marshal(&nodePool)
	//log.FromContext(ctx).V(1).
	//	WithValues("nodePool", string(data)).
	//	WithValues("error", err).
	//	Info("#debug12")
	//time.Sleep(5*time.Millisecond)

	//data, _ = json.Marshal(&instanceTypeMap)
	log.FromContext(ctx).V(1).
		WithValues("instanceTypeMap", len(instanceTypeMap)).
		WithValues("error", err).
		Info("#debug13")
	time.Sleep(5*time.Millisecond)
	...
	//data, _ = json.Marshal(&instanceType)
	//log.FromContext(ctx).V(1).
	//	WithValues("instanceType", string(data)).
	//	WithValues("error", err).
	//	Info("#debug14")
	//time.Sleep(5*time.Millisecond)
	...
	log.FromContext(ctx).V(1).
		WithValues("node",node.Node.Name).
		WithValues("instanceType",instanceType.Name).
		WithValues("pods", len(pods) ).
		WithValues("reschedulablePods", len(lo.Filter(pods, func(p *corev1.Pod, _ int) bool { return pod.IsReschedulable(p) }))).
		WithValues("ReschedulingCost", disruptionutils.ReschedulingCost(ctx, pods) ).
		WithValues("LifetimeRemaining", disruptionutils.LifetimeRemaining(clk, nodePool, node.NodeClaim)).
		WithValues("disruptionCost3", disruptionutils.ReschedulingCost(ctx, pods) * disruptionutils.LifetimeRemaining(clk, nodePool, node.NodeClaim)).
		Info("#debug18")
	time.Sleep(5*time.Millisecond)
