...
func GetCandidates(ctx context.Context, cluster *state.Cluster, kubeClient client.Client, recorder events.Recorder, clk clock.Clock,
	...
	data, _ := json.Marshal(&nodePoolMap)
	log.FromContext(ctx).V(1).
		WithValues("nodePoolMap",string(data)).
		WithValues("error", err).
		Info("#debug8")
	time.Sleep(5*time.Millisecond)

	//data, _ = json.Marshal(&nodePoolToInstanceTypesMap)
	log.FromContext(ctx).V(1).
		WithValues("nodePoolToInstanceTypesMap",len(nodePoolToInstanceTypesMap)).
		WithValues("error", err).
		Info("#debug9")
	time.Sleep(5*time.Millisecond)
	...
	data, _ = json.Marshal(&pdbs)
	log.FromContext(ctx).V(1).

		WithValues("pdbs",string(data)).
		WithValues("error", err).
		Info("#debug10")
	time.Sleep(5*time.Millisecond)
...
func BuildDisruptionBudgets(ctx context.Context, cluster *state.Cluster, clk clock.Clock, kubeClient client.Client, recorder events.Recorder) (map[string]map[v1.DisruptionReason]int, error) {
	// "disruptionBudgetMapping":"{\"dev-vince\":{\"Drifted\":1,\"Empty\":1,\"Underutilized\":1}}"
	...
	for _, nodePool := range nodePoolList.Items {
		...
		data, _ := json.Marshal(&minDisruptionsByReason)
		log.FromContext(ctx).V(1).
			WithValues("nodePool",nodePool.Name).
			WithValues("minDisruptionsByReason",string(data)).
			Info("#debug23")
		time.Sleep(5*time.Millisecond)
